...
class App extends React.Component{
  ...
  constructor(){
    super()
    global.winter = [w0, w1, w2, w3, w4, w5]
    global.summer = [s0, s1, s2, s3, s4, s5, s6, s7,s8]
    global.autumn = [a0, a1, a2, a3]
...
handleClick(event){
  ...
  fetch("https://api.openweathermap.org/data/2.5/weather?q="+city+"&APPID=daae827db6346edc7ba462a1a3a48509")
  ...
  .then(data=>{
    ...
    if (this.state.character.cod==="404"||this.state.cityInput===""){this.setState({weather:"city not found",city:"", cityInput:"",displayWidget:"widget", pictureFrame:"noDisplay"})}
      else{
...
if (this.state.picture===global.winter[global.winter.length-2]){this.setState({navArrow2:"noDisplay"})}
if (this.state.picture===global.summer[global.summer.length-2]){this.setState({navArrow2:"noDisplay"})}
if (this.state.picture===global.autumn[global.autumn.length-2]){this.setState({navArrow2:"noDisplay"})}

if (global.clickCount ===0){this.setState({navArrow1:"noDisplay"})}
...
if (global.clickCount ===0){this.setState({navArrow1:"noDisplay"})}
  ...
  <div className="main">
        <form>
            ...
            <input type="text"  value={this.state.cityInput} onChange ={this.handleChange} id="city" placeholder="Enter Your City Here"required/>
       
          <button className="btn" onClick={this.handleClick}>Check the weather!</button>
        </form>


    <div className={this.state.displayWidget}>

        <b><p>{this.state.city? this.state.city +", "+ this.state.country:""} </p></b>
        <p>{this.state.city? this.state.date:""}</p>
      
       <p>{

           
        this.state.loading ? <h3>loading...</h3> : this.state.weather}</p>
    ...
    <img className={this.state.navArrow1} src={backwardArrow} onClick={this.handleBackwardClick}/>
      <img className="pictures" src={this.state.picture}/>
       <img className={this.state.navArrow2} src ={forwardArrow} onClick={this.handleForwardClick}/>
      <p>{this.state.from} of {this.state.to}</p>
